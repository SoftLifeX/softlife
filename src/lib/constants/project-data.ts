import { projects } from "@/app/assets/projects";
import { StaticImageData } from "next/image";

export interface Project {
  id:       number;
  title:    string;
  img:      StaticImageData;
  bg:       string;
  color:    string;
  href:     string;
  intro:    string;
  stack:    string[];
  disabled?: boolean;
}

export const project: Project[] = [
  {
    id:    0,
    title: "Givtro",
    img:   projects.givtro,
    bg:    "#2466F2",
    href:  "/work/givtro",
    color: "#ffffff",
    intro: "A full-featured Nigerian fintech Mobile Application, built solo across the entire stack.",
    stack: ["React Native", "Expo", "Node.js", "MongoDB", "TypeScript"],
  },
  {
    id:    1,
    title: "Intarakt",
    img:   projects.intarakt,
    bg:    "#F5C200",
    href:  "/intarakt",
    color: "#0d0d0d",
    intro: "Full-stack Social Media Mobile Application built with React Native",
    stack: ["React Native", "Expo", "firebase", "convex", "TypeScript"],
  },
  {
    id:    2,
    title: "Ochi",
    img:   projects.ochi,
    bg:    "#cdea68",
    href:  "https://ochi-website-clone-bysoft.vercel.app/",
    color: "#0d0d0d",
    intro: "High fidelity clone of a modern SaaS website, built to showcase my frontend skills.",
    stack: ["Typescript", "Tailwind", "gsap", "framer-motion", "Next.js"],
  },
  {
    id:    3,
    title: "SupaDupa",
    img:   projects.supadupa,
    bg:    "#163200",
    href:  "https://supadupa-website-bysoft.vercel.app/",
    color: "#c2cabb",
    intro: "Landing page for a SaaS product that helps small businesses sell online.",
    stack: ["Typescript", "Tailwind", "gsap", "framer-motion", "Next.js"],
  },
  {
    id:    4,
    title: "airbnb",
    img:   projects.airbnb,
    bg:    "#0d0d0d",
    href:  "",
    color: "#f5f3ef",
    intro: "A high fidelity clone of the Airbnb website, built to showcase my frontend skills.",
    stack: ["Typescript", "Tailwind", "Next.js", "Prisma"],
    disabled: true,
  },
  {
    id:    5,
    title: "Light",
    img:   projects.light,
    bg:    "#0D1EA2",
    href:  "https://light-website-bysoft.vercel.app/",
    color: "#e8e7e3",
    intro: "A modern, responsive landing page for a SaaS product",
    stack: ["Typescript", "Tailwind", "gsap", "Next.js", "Motion design"],
  },
  {
    id:    6,
    title: "Skyline",
    img:   projects.skyline,
    bg:    "#79BADD",
    href:  "",
    color: "#e8e7e3",
    intro: "Let us help you reach your dream clients today",
    stack: ["Typescript", "Tailwind", "gsap", "framer-motion", "Next.js"],
    disabled: true,
  },
  
  {
    id:    7,
    title: "Flow Party",
    img:   projects.flow,
    bg:    "#B3EB16",
    href:  "https://flow-website-bysoft.vercel.app/",
    color: "#0d0d0d",
    intro: "A modern, responsive landing page for a music festival, built with a focus on smooth animations and user experience.",
    stack: ["Typescript", "Tailwind", "gsap", "framer-motion", "Next.js"],
  },
  {
    id:    8,
    title: "SoftlifeX",
    img:   projects.only,
    bg:    "#1A232E",
    href:  "https://softlifex.vercel.app/",
    color: "#e8e7e3",
    intro: "So why not say HI, after-all \nevery great project starts with that first conversation",
    stack: ["Javascript", "css3", "gsap", "Next.js"],
  },
];